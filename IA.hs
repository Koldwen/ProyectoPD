...
2 usos de evaluación perezosa -- PENDIENTE (2/2)
ejemplo, pilas, colas, map, matrix, array). -- PENDIENTE (2/2)



    La entrega debe contener, en un archivo comprimido (en zip, máximo 15MB):
    El código fuente Haskell debidamente comentado, incluyendo un pequeño readme.txt
    indicando la línea de comando para la compilación (con ghc o un makefile) y la línea de
    comando para lanzar los ejemplos del programa. Se incluirán también el material
    adicional que haga falta para lanzarlos dentro de una carpeta llamada ejemplos.
    Un documento (formato libre, en pdf, máximo 10 páginas) indicando:
     Nombres, apellidos, UVUS y correo electrónico de los componentes del grupo,o
    La temática elegida y el problema a resolver,
    La estructura del código, y cómo y dónde se ha aplicado cada uno de los
